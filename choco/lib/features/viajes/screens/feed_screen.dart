...
import 'package:choco/features/viajes/screens/ranking.dart';
...
class _FeedScreenState extends State<FeedScreen> {
  ...
  Future<void> _precargarBanners() async {
    try {
      ...
      if (mounted) {
        setState(() {
          _bannerPorId.addAll(m);
          _cargandoBanners = false;
        });
      }
    } catch (_) {
      if (mounted) {
        setState(() => _cargandoBanners = false);
      }
    }
  }
  ...
  Future<void> _ctaViaje(BuildContext context, GrupoViajeModel v) async {
    ...
    if (v.faseActual == ViajeFaseProducto.priorizacionCategorias) {
      Navigator.of(context).push(
        MaterialPageRoute<void>(
          builder: (_) => PriorizacionCategoriasScreen(
            viajeId: '${v.id}',
            destinoKey: v.destinoKey,
            nombreViaje: v.nombreViaje,
          ),
  ...
  Future<void> _abrirGastosDelViaje(BuildContext context, GrupoViajeModel v) async {
    ...
    try {
      final lista = await service.fetchViajesPorUsuario(1);
          ...
          child: Column(
            ...
            children: [
              ...
              OutlinedButton(
                ...
                child: Text('Prioriza tu aventura', style: AppFonts.label(13.5, weight: FontWeight.w800)),
              ),
              const SizedBox(height: 8),
              OutlinedButton(
                onPressed: () {
              ...
              OutlinedButton(
                onPressed: () {
                  ...
                  Navigator.of(context).push(
                    MaterialPageRoute<void>(
                      builder: (_) => ItinerarioScreen(itinerarioId: v.id ?? 1, destinoKey: v.destinoKey),
                    ),
                  );
                },
