...
class _ItinerarioHubScreenState extends State<ItinerarioHubScreen> {
  ...
  void initState() {
    super.initState();
    UserSession().addListener(_load);
    _load();
    _refreshTimer = Timer.periodic(const Duration(seconds: 8), (_) {
      if (mounted && UserSession().isLoggedIn) {
        _load(mostrarCargando: false);
      }
    });
  }
  ...
  void dispose() {
    ...
    UserSession().removeListener(_load);
    super.dispose();
  }

  Future<void> _load({bool mostrarCargando = true}) async {
    final user = UserSession().user;
    if (user?.id == null) {
      if (mounted) setState(() { _viajes = <GrupoViajeModel>[]; _banner.clear(); _loading = false; });
      return;
    }
    if (mounted && mostrarCargando) setState(() => _loading = true);
    try {
      ...
      if (mounted) setState(() { _viajes = viajes; _banner..clear()..addAll(m); _loading = false; });
    } catch (_) {
      if (mounted) setState(() { _viajes = <GrupoViajeModel>[]; _banner.clear(); _loading = false; });
    }
  }
