...
import 'package:choco/features/actividades/services/actividades_service.dart';
...
class _ExplorarActividadesSwipeScreenState extends State<ExplorarActividadesSwipeScreen> {
  ...
  final Set<String> _categoriasFiltro = <String>{};
  bool _usarPreferenciasPerfil = true;
  bool _guardandoExploracion = false;
  ...
  Future<_DeckCarga> _cargar({Set<String>? filtrosCategorias, bool? usarPreferenciasPerfil}) async {
    ...
    try {
      ...
      final filtrosActivos = filtrosCategorias ?? _categoriasFiltro;
      final usarPreferencias = usarPreferenciasPerfil ?? _usarPreferenciasPerfil;
      final todas = await const ActividadesService().getActividades(
        destinoKey: key,
        grupoViajeId: grupoId,
        usuarioId: usuarioId,
        categorias: filtrosActivos.toList(),
        usarPreferenciasPerfil: usarPreferencias && filtrosActivos.isEmpty,
      );
      final deck = mezclarDeckActividades(
        todas: todas,
        interesesUsuario: filtrosActivos.toList(),
        limiteInicial: todas.length > 48 ? 48 : todas.length,
      );
      return _DeckCarga(destinoKey: key, todas: todas, deck: deck);
    } catch (e, st) {
      debugPrint('Explorar actividades desde backend: $e\n$st');
      return _DeckCarga(destinoKey: key, todas: const [], deck: const []);
    }
  }
  ...
  void _seguirExplorando() {
    ...
    final nuevo = mezclarDeckActividades(
      todas: restantes,
      interesesUsuario: _categoriasFiltro.toList(),
  ...
  Future<void> _irAEsperaGrupo() async {
    final d = _actual;
    if (d == null || _guardandoExploracion) return;
    final seleccionadas = ExploracionActividadesMemoria.meInteresaIds(_claveMem).toList();
    final n = seleccionadas.length;
    final grupoId = int.tryParse(widget.viajeId ?? '');
    final usuarioId = UserSession().user?.id;

    if (grupoId != null && usuarioId != null) {
      setState(() => _guardandoExploracion = true);
      try {
        await const ActividadesService().confirmarExploracionIndividual(
          grupoViajeId: grupoId,
          usuarioId: usuarioId,
          actividadesInteresIds: seleccionadas,
        );
      } catch (e) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('No pudimos guardar tus actividades todavía. Revisa el backend.', style: AppFonts.body(14))),
          );
        }
        return;
      } finally {
        if (mounted) setState(() => _guardandoExploracion = false);
      }
    }

    if (!mounted) return;
    final esUltimo = false;
  ...
  void _toggleCategoriaFiltro(String categoria) {
    final next = Set<String>.from(_categoriasFiltro);
    if (next.contains(categoria)) {
      next.remove(categoria);
    } else {
      next.add(categoria);
    }
    setState(() {
      ...
      _indice = 0;
      _drag = Offset.zero;
      _usarPreferenciasPerfil = false;
      _carga = _cargar(filtrosCategorias: next, usarPreferenciasPerfil: false);
    });
  }

  void _verTodasLasCategorias() {
    setState(() {
      _usarPreferenciasPerfil = false;
      ...
      _indice = 0;
      _drag = Offset.zero;
      _carga = _cargar(filtrosCategorias: <String>{}, usarPreferenciasPerfil: false);
    });
  }

  void _volverAMisCategorias() {
    setState(() {
      _usarPreferenciasPerfil = true;
      _categoriasFiltro.clear();
      _actual = null;
      _indice = 0;
      _drag = Offset.zero;
      _carga = _cargar(filtrosCategorias: <String>{}, usarPreferenciasPerfil: true);
    });
  }

  void _limpiarFiltrosCategoria() {
    _volverAMisCategorias();
  }

  List<String> _categoriasDisponibles(_DeckCarga d) {
    final categorias = d.todas
                  ...
                  children: [
                    Text(
                      _categoriasFiltro.isEmpty
                          ? 'No encontramos actividades para este destino. Revisa que el backend esté activo y que /actividades/si esté respondiendo.'
                          : 'No encontramos actividades con esos filtros de categoría.',
                      textAlign: TextAlign.center,
                    ...
                    if (_categoriasFiltro.isNotEmpty) ...[
                      const SizedBox(height: 12),
                      OutlinedButton(
                        onPressed: _limpiarFiltrosCategoria,
                        child: Text('Quitar filtros', style: AppFonts.label(13, weight: FontWeight.w800)),
                      ),
                    ],
                  ],
                ),
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _CategoriaSwipeFilters(
                      categorias: _categoriasDisponibles(d),
                      seleccionadas: _categoriasFiltro,
                      usandoPreferenciasPerfil: _usarPreferenciasPerfil && _categoriasFiltro.isEmpty,
                      onToggle: _toggleCategoriaFiltro,
                      onClear: _volverAMisCategorias,
                      onVerTodas: _verTodasLasCategorias,
                    ),
                  ],
                ),
              ),
                          ...
                          children: [
                            ...
                            SizedBox(
                              ...
                              child: OutlinedButton.icon(
                                ...
                                onPressed: _guardandoExploracion ? null : _irAEsperaGrupo,
                                icon: const Icon(Icons.check_circle_outline_rounded, size: 18),
                                label: Text(
                                  widget.viajeId == '7'
                                      ? (_guardandoExploracion ? 'Guardando...' : 'Listo, ver estado del grupo')
                                      : (_guardandoExploracion ? 'Guardando...' : 'Terminar exploración por ahora'),
...
class _CategoriaSwipeFilters extends StatelessWidget {
  final List<String> categorias;
  final Set<String> seleccionadas;
  final bool usandoPreferenciasPerfil;
  final ValueChanged<String> onToggle;
  final VoidCallback onClear;
  final VoidCallback onVerTodas;

  const _CategoriaSwipeFilters({
    required this.categorias,
    required this.seleccionadas,
    required this.usandoPreferenciasPerfil,
      ...
      child: ListView(
        ...
        children: [
          Padding(
            ...
            child: ChoiceChip(
              label: Text('Mis categorías', style: AppFonts.label(12, weight: FontWeight.w700)),
              selected: usandoPreferenciasPerfil,
          ...
          Padding(
            padding: const EdgeInsets.only(right: 8),
            child: ChoiceChip(
              label: Text('Ver todo', style: AppFonts.label(12, weight: FontWeight.w700)),
              selected: !usandoPreferenciasPerfil && seleccionadas.isEmpty,
              onSelected: (_) => onVerTodas(),
            ),
          ),
          ...categorias.map((categoria) {
            final selected = seleccionadas.contains(categoria);
            return Padding(
              padding: const EdgeInsets.only(right: 8),
              child: ChoiceChip(
                label: Text(_capitalizarEtiqueta(categoria), style: AppFonts.label(12, weight: FontWeight.w700)),
                selected: selected,
...
class _CardPreview extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ClipRRect(
      ...
      child: BackendImage(
        source: actividad.imagenAssetPath,
        fit: BoxFit.cover,
